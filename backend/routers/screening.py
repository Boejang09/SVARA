...
router = APIRouter(
    tags=["Screening, History, Notifications"]
)
...
def _screening_response(
    ...
    return ScreeningResponse(
        ...
        raw_output=(
            json.loads(skrining.raw_output)
            if skrining.raw_output
            else None
        ),
...
def analyze_screening(
    ...
    if result.result_type == "retry":
        skrining.status = "retry"
        skrining.nama_penyakit = None
        skrining.heart_status = (
            "Rekaman tidak jelas"
        )
        skrining.confidence = (
            result.confidence
        ...
        return AnalyzeScreeningResponse(
            message=(
                "Rekaman belum cukup jelas. "
                "Silakan lakukan perekaman ulang."
            ),
            data=_screening_response(
                skrining
            ),
        )

    skrining.status = "completed"
    skrining.nama_penyakit = (
        result.prediction
    )
    skrining.risk_analysis = None
    skrining.confidence = (
        result.confidence
    )
    skrining.heart_status = (
