...
**Model `best.pth`:** [Download `best.pth` dari Google Drive](https://drive.google.com/drive/folders/1w96SVztiNoaYeY2ExacHwTSESYLt6zpF?usp=sharing)
...
│   │   ├── app.py
│   │   └── key
│   ├── checkpoints\_combine\_dataset\_artifact/
│   │   └── best.pth
│   └── heart\_sound\_diagnosis\_classification\_drive\_combine\_dataset\_artifact.ipynb
└── README.md
\`\`\`
...
\`\`\`powershell
git clone \<URL\_REPOSITORY\_SVARA>
cd svara\_app
\`\`\`
...
\`\`\`env
DATABASE\_URL=postgresql://postgres\:password\@localhost:5432/svara
\`\`\`
...
JWT\_SECRET\_KEY=GANTI\_DENGAN\_SECRET\_YANG\_AMAN
JWT\_ALGORITHM=HS256
ACCESS\_TOKEN\_EXPIRE\_MINUTES=60

ML\_API\_URL=http\://127.0.0.1:5000/predict
ML\_API\_TIMEOUT\_SECONDS=45
ML\_API\_KEY=GANTI\_DENGAN\_KEY\_ML
\`\`\`
...
\`\`\`text
ml/checkpoints\_combine\_dataset\_artifact/best.pth
\`\`\`
...
\`\`\`powershell
flutter run --dart-define=API\_BASE\_URL=http\://127.0.0.1:8000
\`\`\`

Android fisik melalui jaringan lokal:

\`\`\`powershell
flutter run --dart-define=API\_BASE\_URL=http\://IP\_LAPTOP:8000
\`\`\`

Contoh:

\`\`\`powershell
flutter run --dart-define=API\_BASE\_URL=http\://192.168.0.103:8000
\`\`\`
...
\`\`\`powershell
flutter devices
flutter run -d \<DEVICE\_ID> --dart-define=API\_BASE\_URL=http\://IP\_LAPTOP:8000
\`\`\`
...
\`\`\`powershell
cd mobile\_app
flutter build apk --release --dart-define=API\_BASE\_URL=http\://IP\_LAPTOP:8000
\`\`\`

Contoh:

\`\`\`powershell
flutter build apk --release --dart-define=API\_BASE\_URL=http\://192.168.0.103:8000
\`\`\`

Output:

\`\`\`text
mobile\_app/build/app/outputs/flutter-apk/app-release.apk
\`\`\`
...
POST /api/auth/login
POST /api/records/upload
POST /api/screenings/{screening\_id}/analyze
\`\`\`
...
\`\`\`text
GET    /api/screenings/
GET    /api/screenings/{screening\_id}
POST   /api/screenings/{screening\_id}/analyze
DELETE /api/screenings/{screening\_id}
\`\`\`
...
  "result\_type": "retry",
  "status": "success",
  "segment\_details": ["Artifact", "Artifact"]
}
\`\`\`
...
status        = retry
nama\_penyakit = None
heart\_status  = Rekaman tidak jelas
\`\`\`
...
Pastikan \`ML\_API\_KEY\` pada backend sama dengan key lokal yang digunakan ML API. Jangan tampilkan key sebenarnya.
